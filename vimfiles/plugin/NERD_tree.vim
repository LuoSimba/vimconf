...
if g:running_windows
    let s:nerd_path_slash = '\'
else 
    let s:nerd_path_slash = '/'
endif

" SECTION: Script init stuff(材料) {{{1
"============================================================
...
if s:running_windows    " 在 windows 系统上
...
" ==============
" 得到字符串所表示的绝对路径
function! s:GetAbsolutePath(str)
    if s:running_windows
    ...
    if prependCWD
        return getcwd() . s:nerd_path_slash . a:str
    else 
        return a:str
    endif
endfunction
...
" 过滤只对文件有效, 不过滤文件夹
function! s:toggleFilter()
    let b:NERDTreeFilterEnabled = !b:NERDTreeFilterEnabled
    call s:renderViewSavingPosition()
    call s:centerView()
endfunction

" 他们之间使用 , 分割
function! s:AddFilterExpr(expr)
    let abcList = split(a:expr, ",")
    let exprElement = ''
    let hasOne = 0
    ...
    for i in abcList
        ...
        " otherwise
        if !hasOne
            call add(s:filterExpr, exprElement)
        endif 
    endfor 

    " 从 toggleFilter 函数那里学来的
    call s:renderViewSavingPosition()
    call s:centerView()
endfunction

" 设置要过滤的文件类型
function! s:SetFilterExpr(expr)
    ...
    let tarList = split(a:expr, ",")

    " 先清空
    call remove(s:filterExpr, 0, -1)

    for elem in tarList
        call add(s:filterExpr, '\.' . elem . '$')
    endfor 
...
command! -nargs=? -complete=dir -bar Tree :call s:initNerdTree('<args>')
command! -nargs=? -complete=dir -bar NERDTreeToggle :call s:toggle('<args>')
command! -nargs=0 -bar NERDTreeClose :call s:closeTreeIfOpen()
command! -nargs=1 -complete=customlist,s:completeBookmarks -bar NERDTreeFromBookmark call s:initNerdTree('<args>')
command! -nargs=0 -bar NERDTreeMirror call s:initNerdTreeMirror()
command! -nargs=0 -bar NERDTreeFind call s:findAndRevealPath()
" 我的命令
command! -nargs=1 -bar TreeFilterAdd call s:AddFilterExpr('<args>')
command! -nargs=? -bar TreeFilter    call s:SetFilterExpr('<args>')


" SECTION: 自动命令(Auto commands) {{{1
"============================================================
augroup NERDTree
...
"CLASS: Bookmark {{{2
"============================================================
let s:Bookmark = {}       " 这是一个书签对象啊...
" FUNCTION: Bookmark.activate() {{{3
function! s:Bookmark.activate()
    if self.path.isDirectory        " self 相当于 java 中的 this
        call self.toRoot()
    else
        if self.validate()
...
function! s:Bookmark.CacheBookmarks(silent)
    if filereadable(g:NERDTreeBookmarksFile)
        ...
        if invalidBookmarksFound
            ...
            if !a:silent
                " See :help NERDTreeInvalidBookmarks for info.
                call s:echo(invalidBookmarksFound . " 非法的书签.")
            endif
        endif
        call s:Bookmark.Sort()
    endif
...
" 函数: MenuController.showMenu() {{{3
" start the main loop of the menu and get the 
" user to choose/execute a menu item
...
" 函数: MenuController._echoPrompt() {{{3
...
" 函数: MenuController._handleKeypress(key) {{{3
...
function! s:TreeFileNode.openVSplit()
    ...
    if winnr("$")==#1
        let winwidth = s:NERDTreeWinSize
    endif
...
function! s:TreeFileNode._renderToString(depth, drawText, vertMap, isLastChild)
    ...
    if a:drawText ==# 1
        ...
        "如果该节点是其父节点的末子节点 a:isLastChild
        let treeParts = treeParts . '|'


        "smack the appropriate dir/file symbol on the line before the file/dir
        "name itself
        if self.path.isDirectory          " 如果是文件夹
            if self.isOpen
                let treeParts = treeParts . '~'  " 已经打开的文件夹
            else
                let treeParts = treeParts . '+'  " 尚关闭着的文件夹
            endif
        else                               " 文件
            let treeParts = treeParts . '-'
        endif
...
function! s:Path.extractDriveLetter(fullpath)
    if s:running_windows
        let self.drive = substitute(a:fullpath, '\(^[a-zA-Z]:\).*', '\1', '') " 记录盘符
    else
        let self.drive = ''
    endif
endfunction
...
function! s:Path.getLastPathComponent(dirSlash)
    if empty(self.pathSegments)       " 如果 pathSegments 为空
        return ''
    endif

    let toReturn = self.pathSegments[-1]    " list 的最后一个元素
...
function! s:Path.ignore()
    let lastPathComponent = self.getLastPathComponent(0)   " 参数 0 表示不带末尾的 Slash

    "忽略隐藏文件, unless instructed to
    if b:NERDTreeShowHidden ==# 0
        for i in s:hiddenExpr
    ...
    if !b:NERDTreeFilterEnabled    " 禁用文件过滤
        return 0                   " 那么这些文件都不应该被忽略
    endif 

    "filter out the user specified paths to ignore
    " 如果启用文件过滤
    for i in s:filterExpr       " 读取文件类型列表
        if lastPathComponent =~? i  " 比较的时候, 请忽略大小写
            return 0
        endif
    endfor
...
function! s:Path.isUnder(path)
    ...
    let this = self.str()
    let that = a:path.str()
    return stridx(this, that . s:nerd_path_slash) == 0
endfunction
...
function! s:Path.New(path)
    ...
    call newPath.readInfoFromDisk(s:GetAbsolutePath(a:path))
...
function! s:Path.readInfoFromDisk(fullpath)
    call self.extractDriveLetter(a:fullpath)  " --> self.drive

    let fullpath = s:Path.WinToUnixPath(a:fullpath) " 转 windows 路径为 unix 路径

    if getftype(fullpath) ==# "fifo"   " 该文件所代表的对象
        throw "NERDTree.InvalidFiletypeError: Cant handle FIFO files: " . a:fullpath
    endif

    let self.pathSegments = split(fullpath, '/')      " 分割字符串, 得到 list

    let self.isReadOnly = 0
    if isdirectory(a:fullpath)   " 是否为文件夹
        let self.isDirectory = 1
    elseif filereadable(a:fullpath)  " 是否可读
...
function! s:Path._strForGlob()
    ...
    let toReturn = lead . join(self.pathSegments, s:nerd_path_slash)
...
function! s:Path._str()
    ...
    return lead . join(self.pathSegments, s:nerd_path_slash)
endfunction
...
function! s:initNerdTree(name)
    ...
    if s:Bookmark.BookmarkExistsFor(a:name)
        ...
        if dir =~ '^\.'
            let dir = getcwd() . s:nerd_path_slash . dir
        endif
        let dir = resolve(dir)
...
function! s:initNerdTreeInPlace(dir)
    ...
    "我们需要一个唯一的名字 for each secondary tree buffer
    " 来确保它们是独立的
...
function! s:tabpagevar(tabnr, var)
    let currentTab = tabpagenr()    " 当前标签号, 从 1 开始
    let old_ei = &ei
    set ei=all

    exec "tabnext " . a:tabnr   | " 转到第几个标签
    let v = -1
    if exists('t:' . a:var)
        exec 'let v = t:' . a:var
    endif
...
function! s:createTreeWin()
    ...
    "throwaway buffer options
    setlocal noswapfile          " 没有交换文件
    setlocal buftype=nofile
    setlocal nowrap
    setlocal foldcolumn=0
    setlocal nobuflisted
    setlocal nospell
    setlocal nonu                " 不要显示行号

    iabc <buffer>

    setlocal cursorline

    call s:setupStatusline()     " 设置状态栏

    call s:bindMappings()
    setfiletype nerdtree        " 设置文件类型
    " syntax highlighting
    if has("syntax") && exists("g:syntax_on")
        call s:setupSyntaxHighlighting()
    endif
...
function s:dumpHelp()
    ...
    if 1
        ...
        \ ("; ". g:NERDTreeMapActivateNode .": open in prev window\n") :
        ...
        \ "; ". g:NERDTreeMapOpenInTabSilent .": 在新标签页打开(静)\n" .
        \ "; middle-click,\n" .
        \ "; ". g:NERDTreeMapOpenSplit .": open split\n" .
        \ "; ". g:NERDTreeMapPreviewSplit .": preview split\n" .
        \ "; ". g:NERDTreeMapOpenVSplit .": open vsplit\n" .
        \ "; ". g:NERDTreeMapPreviewVSplit .": preview vsplit\n"
        ...
        \ . "; ". g:NERDTreeMapCloseChildren .": close all child nodes of\n"
        \ . ";    current node recursively\n"
        \ . "; middle-click,\n"
        \ . "; ". g:NERDTreeMapOpenExpl.": explore selected dir\n"

        let @h .= ";\n; ----------------------------\n"
        \ . "; 书签列表映射~\n"
        \ . "; double-click,\n"
        \ . "; ". g:NERDTreeMapActivateNode .": open bookmark\n"
        \ . "; ". g:NERDTreeMapOpenInTab.": open in new tab\n"
        \ . "; ". g:NERDTreeMapOpenInTabSilent .": open in new tab silently\n"
        \ . "; ". g:NERDTreeMapDeleteBookmark .": delete bookmark\n"

        let @h .= ";\n; ----------------------------\n"
        \ . "; 树导航映射~\n"
        \ . "; ". g:NERDTreeMapJumpRoot .": 转到根节点\n"
        \ . "; ". g:NERDTreeMapJumpParent .": 转到父节点\n"
        \ . "; ". g:NERDTreeMapJumpFirstChild  .": 转到首子节点\n"
        \ . "; ". g:NERDTreeMapJumpLastChild   .": 转到末子节点\n"
        \ . "; ". g:NERDTreeMapJumpNextSibling .": 下一节点\n"
        \ . "; ". g:NERDTreeMapJumpPrevSibling .": 上一节点\n"

        let @h .= ";\n; ----------------------------\n"
        \ . "; 文件系统映射~\n"
        \ . "; ". g:NERDTreeMapChangeRoot .": 切换根目录\n"
        \ . "; ". g:NERDTreeMapUpdir .": 树根向上一级\n"
        ...
        let @h .= ";\n; ----------------------------\n"
        \ . "; 树过滤映射~\n"
        \ . "; ". g:NERDTreeMapToggleHidden .": 显示隐藏文件 (" . (b:NERDTreeShowHidden ? "on" : "off") . ")\n"
        \ . "; ". g:NERDTreeMapToggleFilters .": 文件类型过滤(新) (" . (b:NERDTreeFilterEnabled ? "on" : "off") . ")\n"
        \ . "; ". g:NERDTreeMapToggleFiles .": 显示文件 (" . (b:NERDTreeShowFiles ? "on" : "off") . ")\n"
        \ . "; ". g:NERDTreeMapToggleBookmarks .": 书签 (" . (b:NERDTreeShowBookmarks ? "on" : "off") . ")\n"

        "add quickhelp entries for each custom key map
        if len(s:KeyMap.All())
            let @h .= ";\n; ----------------------------\n"
            let @h .= "; Custom mappings~\n"
            for i in s:KeyMap.All()
                let @h .= "; ". i.key .": ". i.quickhelpText ."\n"
            endfor
        endif
...
function! s:renderView()
    ...
    "删除缓冲区所有行 (being careful not to clobber a register)
    silent 1,$delete _

    call s:dumpHelp()

    "删除帮助之前的空行, 并在帮助之后添加一空行
    call setline(line(".")+1, "")
    call cursor(line(".")+1, col("."))
...
function! s:renderViewSavingPosition()
    ...
    "out of nodes
    while currentNode != {} && currentNode.path.ignore() && !currentNode.isRoot()
        let currentNode = currentNode.parent
    endwhile
...
function s:setupStatusline()
    let &l:statusline = s:NERDTreeStatusline
endfunction
...
function! s:setupSyntaxHighlighting()
    ...
    hi treeRoot      guifg=Red
    hi treeHelp      guifg=Red
    hi treeHelpTitle guifg=Green
...
function! s:toggle(dir)
    if s:treeExistsForTab()        " 是否存在
        if !s:isTreeOpen()         " 是否已经打开
            call s:createTreeWin()
            if !&hidden
                call s:renderView()
            endif
...
function! s:bindMappings()
    ...
    nnoremap <silent> <buffer> m :call <SID>showMenu()<cr>
...
function s:showMenu()
    ...
    let luo_temp_var = s:MenuItem.AllEnabled()
    
    let luo_mc = s:MenuController.New(luo_temp_var)
    
    call luo_mc.showMenu()
endfunction " }}}
...
" vim: set sts=4 et fdm=marker fileencoding=cp936:
